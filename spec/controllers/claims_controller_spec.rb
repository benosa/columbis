...
describe ClaimsController do
  def create_prerequisites
    ...
    @country = Factory(:country, :name => 'Turkey')
    @resort = Factory(:resort)
    @city = Factory(:city)
  ...
  def create_claim
    @claim = Factory(:claim, :user_id => @manager.id, :office_id => @office.id,
                  :country_id => @country.id, :resort_id => @resort.id, :city_id => @city.id, :apllicant_id => @applicant.id)
  end
